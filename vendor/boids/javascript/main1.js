function main(){
  ...
  for(var i = 0; i<10; i++)
   ...
   this.prueba = function(){
       while(fixed_target.geo_data.position.get_coord(0) <= 300){

	   alert("bene")
	       if( fixed_target.geo_data.position.get_coord(0) >= 300)
		   fixed_target.geo_data.velocity = new Vector(-10,0)
       }

       if(fixed_target.geo_data.position.get_coord(0) <= 100)
	   fixed_target.geo_data.velocity = new Vector(10,0)
	       if(fixed_target.geo_data.position.get_coord(0) >= 240)
		   alert("bieeen")
...
if( fixed_target.geo_data.position.get_coord(0) >= 300)
  fixed_target.geo_data.velocity = new Vector(-60,0)
else
    if(fixed_target.geo_data.position.get_coord(0) <= 100)
	fixed_target.geo_data.velocity = new Vector(60,0)

   }
   setInterval(direccion,30);
   w.start()
